...
$baseURL = "http://apm.rsuwahidinmojokerto.com:8191/";
...
if($phone!='6281335537942'){ // nomor untuk debugging
	if($phone!=""){
		...
		// echo "Mohon maaf, orangnya sedang dalam perbaikan";die();
		die();
	}
}else{
		...
		if(strlen($noBpjs)==13){
			...
			if($cekRespon==200){ // jika cek noka berhasil, langsung cek rujukan
				...
				if($resOK = $resRujuk->code==200){
					...
					$msg = "Silahkan masukkan Nomor Rujukan/Nomor Referensi.\n";
...
if(strlen($waText)==11 && $rows['pasien_baru']==false && $statusChat==20){
	...
	if($execCekPasLama->num_rows>0){
		...
		if($nik=="" || empty($nik)){
			$msg .= "Silahkan masukkan NIK Anda.\n";
...
if(is_numeric($waText) && $ifKodeDokter){
	...
	$queryDokter = "SELECT * FROM dokter_bridg WHERE kodedokter='$waText'";
	...
	if($execDokter->num_rows>0){
		...
		if($kodeDokter==""){
			...
			$resFilterDokter = filterDokter($resJadDok,$kodeDokter);
			if(!empty($resFilterDokter) || count($resFilterDokter)>0){
				...
				if($rowsDapas['is_pasien_baru']==0){
					...
					if($cek=='INT' || $cek=='KLT' || $cek=='JAN'){
						$counter = "Counter A";
					}elseif($cek=='URO'||$cek=='BSY'||$cek=='ORT'||$cek=='BED'||$cek=='OBG'||$cek=='017'){
						$counter = "Counter B";
					}elseif($cek=='IRM'||$cek=='PAR'||$cek=='SAR'||$cek=='GIZ'||$cek=='ANT'){
						$counter = "Counter C";
					}elseif($cek=='THT'||$cek=='MAT'||$cek=='ANA'||$cek=='JIW'||$cek=='VCT'){
						$counter = "Counter D";
					}elseif($cek=='GIG'){
						$counter = "Counter E";
					}
...
if($waText=='reset' && $statusChat>0 && ($rows['pasien_baru']==false || $rows['pasien_baru']==true)){
	$msg = msgWelcome();
...
function storeRsuRegister($idPsn,$wablas,$dbrsud){
	...
	$result = $storeRsuRegister->storeRsuRegister(new Request($data));
	return $result;
}
...
function msgWelcome(){
	...
	$msg .= "Hotline 0815257200088 untuk mendapatkan bantuan apabila ada kendala.\n";
