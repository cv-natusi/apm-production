...
// file_put_contents('json.txt', print_r($data,1));
// die();
...
// 	echo json_encode('testing',JSON_PRETTY_PRINT);
// 	die();
// }
// $phone2 = $phone;
// if($phone=='6281335537942'){
...
// INFO SERVER MAINTENACE
// if($phone!='6281335537942'){ // nomor untuk debugging
// 	if($phone!=""){
// 		// echo "Mohon maaf, server sedang dalam perbaikan hingga pukul 16:00 WIB";die();
		// echo "Mohon maaf, server sedang dalam perbaikan.";die();
// 		// echo "Mohon maaf, server sedang dalam perbaikan";die();
...
// $baseURL = "http://117.102.75.166:8191/public/";
...
if($result->num_rows<1){// INFO PENDAFTARAN START
	$msg = msgWelcome();
}
...
if(is_numeric($waText) && $statusChat==1 && $waText=='1'){
	if(($rows['pasien_baru']=="" || $rows['pasien_baru']==1) && $rows['pasien_id']==""){
		...
		// 		// $upd = "UPDATE bot_pasien SET cara_bayar='tes' WHERE phone='$phone' AND tanggalChat='$dateNow'";
		// 		// $ex = mysqli_query($wablas,$upd);
		// 		// echo $ex."\n";
		// 		// mysqli_rollback($wablas);
		// 		// // mysqli_commit($wablas);
		// 		// $q = "SELECT * FROM bot_pasien WHERE phone='$phone' AND tanggalChat='$dateNow'";
		// 		// $ex = mysqli_query($wablas,$q);
		// 		// // echo json_encode($ex,JSON_PRETTY_PRINT);
		// 		// echo json_encode($ex->fetch_assoc(),JSON_PRETTY_PRINT);
		// 		// die();
...
// INPUT NOMOR KARTU END

// INPUT TANGGAL BERKUNJUNG START
			...
			if($jenisPembayaran=='BPJS' && !in_array($rowsDapas['jenisRujukan'], ['kontrol','internal']) && $rowsDapas['rujukanAktif'] < $tglBerobat){
				...
				if($execQGetAntri->num_rows>0){
					...
					if($resBatpas->num_rows>0){
						...
						$poli = "SELECT tp.NamaPoli,mp.kdpoli_rs,mp.kdpoli FROM mapping_poli_bridging AS mp JOIN tm_poli AS tp ON mp.kdpoli_rs=tp.KodePoli WHERE mp.kdpoli NOT IN ('ALG','UGD','ANU','GIG') GROUP BY mp.kdpoli_rs ORDER BY tp.KodePoli ASC"; # GIG=="poli gigi dokter umum"
...
// INPUT TANGGAL BERKUNJUNG END


// if($phone=='6281335537942'){
// 	echo json_encode($rows,JSON_PRETTY_PRINT);die();
// }


// INPUT NORM(pasien lama) START
...
// INPUT NORM(pasien lama) END

// INPUT NIK PASIEN LAMA(jika tm_customer(NoKtp) kosong) START
...
// INPUT NIK PASIEN LAMA(jika tm_customer(NoKtp) kosong) END

$um = $waText=='umum';
$bp = $waText=='bpjs';
$al =  $waText=='asuransilain';
// INPUT JENIS PEMBAYARAN(pasien lama) START
...
// INPUT JENIS PEMBAYARAN(pasien lama) END

// INPUT NOREF/RUJUKAN(pasien bpjs) START
...
// INPUT NOREF/RUJUKAN(pasien bpjs) END
...
if(is_numeric($waText) && ($ifPoli)){
	...
	// 	die("DWI ALIM");
	// }
	// if($phone=='6281335537942' || $waText==4){
	// if($waText==4){
	...
	// 	die();
	// }
	$ignorePoli = "mp.kdpoli NOT IN ('ALG','UGD','ANU','GIG')";
	$poli = "
		SELECT
	...
	if($rowsDapas['kodePoli']==NULL && $rows['data_diri']==true && ($linkPasOld || $linkPasNew)){
		...
		if(array_key_exists($index, $arrPoli)){
			...
			// if($phone==='6281335537942'){
			// 	echo json_encode($resJadDok,JSON_PRETTY_PRINT);
			// 	// echo json_encode($arrKodeDokter,JSON_PRETTY_PRINT);
			// 	die();
			// }
			if($resJadDok['metaData']->code==200) {
...
// INPUT KODE DOKTER START / KONFIRMASI
...
// INPUT KODE DOKTER END / KONFIRMASI

// if($statusChat==99 && $phone2=='6281335537942'){
if($statusChat==99){
...
function msgWelcome(){
	...
	$msg .= "*Pelayanan IGD & POLI HD tetap buka 24 JAM*\n\n";
