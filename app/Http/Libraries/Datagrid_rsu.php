...
class Datagrid_rsu {
	...
	{
		$select = $table->select( \DB::connection('dbrsud')->raw($param['select']) )
	...
	{
		$select_total = $table->select( \DB::connection('dbrsud')->raw('sql_calc_found_rows ' . $param['select']) );
		...
		$select_total = \DB::connection('dbrsud')->select( \DB::connection('dbrsud')->raw("select found_rows() as total;") );
	...
	{
		$table = $modifier( \DB::connection('dbrsud')->table($param['table']) );
