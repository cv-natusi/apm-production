...
class Datagrid_rsudlain {
	...
	{
		$select = $table->select( \DB::connection('dbrsudlain')->raw($param['select']) )
	...
	{
		$select_total = $table->select( \DB::connection('dbrsudlain')->raw('sql_calc_found_rows ' . $param['select']) );
		...
		$select_total = \DB::connection('dbrsudlain')->select( \DB::connection('dbrsudlain')->raw("select found_rows() as total;") );
	...
	{
		$table = $modifier( \DB::connection('dbrsudlain')->table($param['table']) );
