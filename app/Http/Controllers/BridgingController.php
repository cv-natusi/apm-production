...
class BridgingController extends Controller{
	...
	public function main(Request $request){
		...
		if (!empty($request->id)) {
			if (!empty($request->metode)) {
				if ($request->metode == 'WA') {
					$this->data['ambil'] = 'WA';
					$this->data['antrian'] = DB::connection('mysql')->table('bot_data_pasien')->where('cust_id', $request->id)->first();
					$this->data['noka'] = $this->data['antrian']->nomor_kartu;
					$this->data['noMr'] = $this->data['antrian']->KodeCust;
				} else {
					$this->data['ambil'] = 'SIMAPAN';
					$this->data['antrian'] = DB::connection('mysql')->table('pasien_baru_temporary')->where('id_pas', $request->id)->first();
					$this->data['noka'] = $this->data['antrian']->bpjs;
					$this->data['noMr'] = $this->data['antrian']->no_rm;
				}
			} else {
				$this->data['ambil'] = 'KIOSK';
				$this->data['antrian'] = Antrian::with('tm_customer')->where('id', $request->id)->first();
				$this->data['noka'] = $this->data['antrian']->nomor_kartu;
				$this->data['noMr'] = $this->data['antrian']->no_rm;
			}
		}else{
		...
		# pengecekan no rujukan
		$this->data['is_skdp'] = '';
		if ($this->data['antrian']) {
			$preNoRujuk = substr($this->data['antrian']->nomor_referensi, 12,1);
			$this->data['is_skdp'] = $preNoRujuk=='K'?'Y':'N'; # jika prefix = K berarti no skdp 
		}
		...
		if($valid->fails()){
			...
			if ($err) {
				...
				if ($respon['response'] == null) {
					...
					// PENGECEKAN DATA SUDAH ADA/TIDAK
					$cekReg = Rsu_Register::where('No_RM', $request->no_rm)->whereDate('Tgl_Register','=', date('Y-m-d'))->first();
					if (!empty($cekReg)) {
						$reg = $cekReg;
						$nourut = $reg->No_Register;
					} else {
						$reg = new Rsu_Register; // db rsu
						if($urut){
					...
					if($reg){
						if ($request->metode = 'KIOSK') {
							// UPDATE KE TABEL ANTRIAN
							$getData = DB::connection('mysql')->table('antrian')->where('id', $idantri)->update(['sudah_buat_sep'=>'Ya','No_Register'=>$nourut]);
						} else if ($request->metode = 'SIMAPAN') {
							// UPDATE KE TABEL BOT DATA PASIEN
							$getData = DB::connection('mysql')->table('bot_data_pasien')->where('cust_id', $cust_id)->update(['sudah_buat_sep'=>'Ya','nosep'=>$reg->NoSEP]);
						} else if ($request->metode = 'WA') {
							// UPDATE KE TABEL PASIEN BARU TEMPORARY
							$getData = DB::connection('mysql')->table('pasien_baru_temporary')->where('id_pas', $id_pas)->update(['sudah_buat_sep'=>'Ya','nosep'=>$reg->NoSEP]);
						}
					}
	...
	public function cetaksepservice(Request $request){
		// if (!empty($request->idAntri)) {
		// 	$idAntri = $request->idAntri;
		// }//awal
		if (!empty($request->metode)) {
			if ($request->metode == 'KIOSK') {
				$idAntri = $request->idAntri;
		...
		if ($result['metaData']->code == 200) {
			...
			$customer = rsu_customer::select('Telp')->where('KodeCust',$result['response']->peserta->noMr)->first();
			$data['sepValue'] = [
				// 'jam' => date('d-m-Y H:i:s'),
				'jam' => $dateTimeCur,
				'no_hp' => ($customer && $customer->Telp) ? $customer->Telp : '00000000',
			...
			if ($request->metode == 'KIOSK') {
				$getAntrian = DB::connection('mysql')->table('antrian')->where('id', $idAntri)->update(['sudah_cetak_sep' => 'Ya']);
			} elseif ($request->metode == 'WA') {
				$getAntrian = DB::connection('mysql')->table('bot_data_pasien')->where('cust_id', $idAntri)->update(['sudah_cetak_sep' => 'Ya']);
			} elseif ($request->metode == 'SIMAPAN') {
				$getAntrian = DB::connection('mysql')->table('pasien_baru_temporary')->where('id_pas', $idAntri)->update(['sudah_cetak_sep' => 'Ya']);
			}
	...
	public function cariHistorySEP(Request $request){
		// return $request->all();
		$dateNow = date('Y-m-d');
		$dateAgo =  date('Y-m-d', strtotime("-89 day", strtotime($dateNow)));
	...
	public function getDPJP(Request $request){
		...
		date_default_timezone_set('Asia/Jakarta');
		// $url = "https://dvlp.bpjs-kesehatan.go.id/vclaim-rest/Rujukan/Peserta/".$request->nobpjs; // url develop
		if($request->kdpoli == 'IGD'){
			$url = $this->url."/referensi/dokter/pelayanan/1/tglPelayanan/".$request->tgl."/Spesialis/".$request->kdpoli; // url rilis
		} else{
			if ($request->kdpoli == 'HDL') {
				$request->kdpoli = 'INT';
			}
