...
class MstPoliController extends Controller{
	...
	public function store(Request $request){
		$request->merge([
			'poli' => strtoupper($request->poli),
			'kodeawalan' => strtoupper($request->kodeawalan),
		]);
		if(($cekDuplikat = KodeAwalanPoli::where('kode_awal', $request->kodeawalan)->first()) && $cekDuplikat->kdpoli_rs != $request->id){
			$poli = rsu_poli::where('KodePoli',$cekDuplikat->kdpoli_rs)->first();
			$namaPoli = $poli ? $poli->NamaPoli : 'poli lain';
			return [
				'status' => 'error',
				'code' => 400,
				'message' => "Kode sudah digunakan untuk $namaPoli",
			];
		}
		$cek = 'lama';
		if(!($kodeAwalan = KodeAwalanPoli::where('kdpoli_rs', $request->id)->first())){
			$cek = 'baru';
