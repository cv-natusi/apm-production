						...
						<div class="col-lg-6">
							...
							@if($tglRujukan != '')
								<div id="showTglRujuk">
									...
									<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 col-lg-offset-4 col-md-offset-4 intevalRujuk"><i>Rujukan <b>{!! $intervalRujuk !!}</b> Hari</i></div>
								</div>
							@endif
							<input type="hidden" name="nmPoli" value="{{ $nmPoli }}" id="nmPoli">
							<input type="hidden" name="kdSelectedPoli" value="{{ $kdPoli }}" id="kdSelectedPoli">
							<input type="hidden" name="kdPoliRiwayat" value="{{ $kdPoliRiwayat }}" id="kdPoliRiwayat">
						...
						<div class="col-lg-6">
							...
							@if($jenis_pendaftaran == 'BPJS')
								...
								<div class="showDokter"  <?php //if ($record == 'nothing') { echo "style='display:none;'";} ?>>
									<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Dokter Perujuk</label>
									<label class="col-lg-6 col-md-6 col-sm-6 col-xs-6" id="showViewDokter">: {{ $dokterSebelum }}</label>
									<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 btnUpdateDokAlter" <?php //if ($record == 'nothing') { echo "style='display:none;'";} ?>>
										<a href="javascript:void(0)" onclick="updateDokter()" class="btn btn-info btn-xs">
											<i class="fa fa-pencil"></i> Ubah
										</a>
									</div>
									<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 listDokAlter pull-left" <?php //if ($record == 'nothing') { echo "style='display:none;'";} ?> style='display:none;'>
										<select name="id_dokterDpjp" class="" id="pilihDokterDpjp" style="width: 100%;">
											<option>.: Pilih Dokter DPJP :. </option>
											@foreach($dokAlters as $dokAlter)
												<option value="{{ $dokAlter->dokter }}" <?php if($dokAlter->dokter == $dokterSebelum){echo "selected";} ?>>{{ $dokAlter->dokter }} - {{ $dokAlter->poli }}</option>
											@endforeach
										</select>
									</div>
									<a href="javascript:void(0)" onclick="changeDokter()" class="btn btn-success btn-xs btnSaveDokAlter pull-right m-r-20"  style="display: none;background: #5cb85c">
										<i class="fa fa-save"></i> save
									</a>
								</div>
								<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Kode DPJP</label>
								<div class="col-lg-8 col-md-8 col-sm-6 col-xs-6">
									<b>: </b><input type="text" name="kodeDPJP" value="{{ $kodeDPJP }}" id="kodeDPJP" readonly>
								</div>
								<div class="clearfix m-b-5"></div>
								<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Nomer SKDP</label>
								<div class="col-lg-8 col-md-8 col-sm-6 col-xs-6">
									<b>: </b><input type="text" name="noSurat" value="" id="noSurat">
								</div>
								<div class="clearfix m-b-5"></div>
								<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Tujuan Kunjungan<font style="color:red;">*</font></label>
								<div class="col-lg-8 col-md-8 col-sm-6 col-xs-6">
									<b>: </b><select name="tujuan_kunjugan" onchange="tujuan_kunjungan(this)" id="tujuan_kunjugan_text" class="">
								...
								<input type="hidden" name="kdPoliSblm" value="@{{ $kdPoliSebelum }}" id="kdPoliSebelum">
							@endif
						</div>
...
<script type="text/javascript">
	...
	function updateDokter() {
		$('.btnUpdateDokAlter').hide();
		$('.showViewDokter').hide();
		$('.listDokAlter').show();
		$('.btnSaveDokAlter').show();
	}

	function changeDokter() {
		var dokter = $('#pilihDokterDpjp').val();
		$.post("{!! route('cekDokterDpjp') !!}",{dokter:dokter}).done(function(result){
			if(result.status == 'success'){
				$('#showViewDokter').html(result.data.namaDPJP);
				$('#kodeDPJP').val(result.data.kodeDPJP);
				$('#namaDokterDPJP').val(result.data.namaDPJP);

				$('.btnUpdateDokAlter').show();
				$('.showViewDokter').show();
				$('.listDokAlter').hide();
				$('.btnSaveDokAlter').hide();
			}else{
				swal({
					title : "Maaf !!",
		...
		if (noRujuk != '' && noRujuk != '0') {
			$.post("{!! route('reloadNoRujukan') !!}",{noBpjs:noBpjsPas,noRM:noRM,noRujuk:noRujuk}).done(function(result){
				if(result.status == 'success'){
					...
					// }
					$('#kdSelectedPoli').val(result.data.kdPoli);
					$('#kdPoliRiwayat').val(result.data.kdPoliRiwayat);
					...
					var interRujuk = '<i>Rujukan <b>'+result.data.intervalRujuk+'</b> Hari</i>';
					$('.intevalRujuk').html(interRujuk);
				}else{
					swal({
						title : "Maaf !!",
						text: result.messages,
										...
										// console.log(data);
											if(data.status == 'success'){
												...
												// for (var p = 0; p < 3; p++) {
													...
													cetak += '<td '+widthIsi1+' '+stIsi+'>: '+data.data.sepValue["fakses"]+'</td>';
    ...
    function cetakKetiga() {
		...
		function tujuan_kunjungan(e) {
			...
			if (e.value == 1) {
				...
				var selects = '';
				selects += `<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Flag Prosedur*</label><div class="col-lg-8 col-md-8 col-sm-6 col-xs-6"><b>:</b><select name="prosedur_bpjs" id="prosedur_bpjs" class="">`;
				flagProcedure.forEach(element => {
					selects += `<option value="${element[0]}">${element[1]}</option>`;
				});
				selects += `</select></div><div class="clearfix m-b-5"></div>`;
				// SELECT PENUNJANG
				selects += `<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6">Penunjang</label><div class="col-lg-8 col-md-8 col-sm-6 col-xs-6"><b>:</b><select name="penunjang_bpjs" id="penunjang_bpjs" class="">`;
				penunjang.forEach(element => {
					selects += `<option value="${element[0]}">${element[1]}</option>`;
				});
				selects += `</select></div><div class="clearfix m-b-5"></div>`;
				selects += `<label class="col-lg-4 col-md-4 col-sm-6 col-xs-6" style="font-size: 13px;">Assesment Pelayanan</label><div class="col-lg-8 col-md-8 col-sm-6 col-xs-6"><select name="assesment_bpjs" id="assesment_bpjs" class="">`;
