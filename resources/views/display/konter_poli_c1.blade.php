...
<body class="hold-transition lockscreen">
						...
						<div class="col-md-12">
							<div class="card bg-success border border-secondary" style="height:100%">
								<div class="embed-responsive embed-responsive-21by9" style="height:100%">
									{{-- <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/{{$data['link']}}?&autoplay=1&loop=1&rel=0&showinfo=0&color=white&iv_load_policy=3&playlist={{$data['link']}}" allowfullscreen></iframe> --}}
									<div id="playerId"></div>
								</div>
							</div>
						</div>
	...
	<script type="text/javascript">
		...
		function loadDataCounterSaatIni() {
			...
			$.post("{{ route('antrianCounter') }}", data ).done(function(response) {
				if(response.status == 'success'){
					let html = "";
								...
								html +=`
										<div class="card border border-secondary" style="height: 100%">
											<div class="card-body text-center p-2">
												<h1 class="m-0 text-secondary font-weight-bold" style="font-size: 5rem;">
													${val[0].mapping_poli_bridging.tm_poli.NamaPoli}
												</h1>
												<h1 class="m-0 font-weight-bold" style="font-size:15rem !important;">
													${val[0].nomor_antrian_poli}
												</h1>
												<h2 class="m-0 font-weight-bold" style="font-size: 4rem;">
													Dipanggil
												</h2>
											</div>
										</div>
