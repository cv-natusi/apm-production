				...
				<div class="box box-primary main-layer">
					<div class="col-md-4 col-sm-4 col-xs-12 form-inline main-layer" style='padding:5px'>
						<button type="button" class="btn btn-sm btn-primary" id="btn-add">
							<span class="fa fa-plus"></span> &nbsp New Pola Hidup
						</button>
					</div>
					<!-- Search -->
...
@section('script')
	...
	<script type="text/javascript">
		var datagrid = $("#datagrid").datagrid({
			url                     : "{!! route('PolaHidupDatagrid') !!}",
			primaryField            : 'id_pola_hidup_sehat', 
			rowNumber               : true, 
			rowCheck                : false, 
			...
			columns                 : [
				{field: 'judul', title: 'Judul', editable: false, sortable: true, width: 450, align: 'left', search: true},
		...
		$('#btn-add').click(function(){
			$('.loading').show();
			$('.main-layer').hide();
			$.post("{!! route('formAddPolaHidup') !!}").done(function(data){
		...
		function view(rowIndex) {
			var id = datagrid.getRowData(rowIndex).id_pola_hidup_sehat;
			$.post("{!! route('detailPolaHidup') !!}", {id:id}).done(function(data){
				if(data.status == 'success'){
		...
		function updated(rowIndex){
			...
			$('.loading').show();
			$('.main-layer').hide();
			$.post("{!! route('formUpdatePolaHidup') !!}", {id:rowData.id_pola_hidup_sehat}).done(function(data){
				if(data.status == 'success'){
					$('.loading').hide();
					$('.other-page').html(data.content).fadeIn();
				} else {
		...
		function deleted(rowIndex) {
			...
			swal(
				{
					title: "Apa anda yakin Menghapus Pola Hidup Ini?",
					text: "Pola Hidup akan dihapus dari sistem dan tidak dapat dikembalikan!",
					type: "warning",
				...
				function(){
					$.post("{!! route('deletePolaHidup') !!}", {id:rowData.id_pola_hidup_sehat}).done(function(data){
						if(data.status == 'success'){
							datagrid.reload();
							swal("Success!", "Pola Hidup Berhasil Dihapus", "success");
						}
					});
				}
			);
