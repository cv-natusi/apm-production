...
@section('script')
	...
	<script type="text/javascript">
		var datagrid = $("#datagrid").datagrid({
			url                     : "{!! route('bantuanDatagrid') !!}",
			primaryField            : 'id_bantuan', 
			rowNumber               : true, 
			rowCheck                : false, 
			...
			columns                 : [
				{field: 'nama_bantuan', title: 'Nama Penyakit', editable: false, sortable: true, width: 450, align: 'left', search: true},
		...
		$('#btn-add').click(function(){
			$('.loading').show();
			$('.main-layer').hide();
			$.post("{!! route('formAddbantuan') !!}").done(function(data){
		...
		function view(rowIndex) {
			var id = datagrid.getRowData(rowIndex).id_bantuan;
			$.post("{!! route('detailbantuan') !!}", {id:id}).done(function(data){
				if(data.status == 'success'){
		...
		function updated(rowIndex){
			...
			$('.loading').show();
			$('.main-layer').hide();
			$.post("{!! route('formUpdatebantuan') !!}", {id:rowData.id_bantuan}).done(function(data){
				if(data.status == 'success'){
					$('.loading').hide();
					$('.other-page').html(data.content).fadeIn();
				} else {
		...
		function deleted(rowIndex) {
			...
			swal(
				{
					title: "Apa anda yakin Menghapus Data Bantuan Ini?",
					text: "Data Bantuan akan dihapus dari sistem dan tidak dapat dikembalikan!",
					type: "warning",
				...
				function(){
					$.post("{!! route('deletebantuan') !!}", {id:rowData.id_bantuan}).done(function(data){
						if(data.status == 'success'){
							datagrid.reload();
							swal("Success!", "Data Bantuan Berhasil Dihapus", "success");
						}
					});
				}
			);
