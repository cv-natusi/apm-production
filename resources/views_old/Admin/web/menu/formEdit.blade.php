<div class="box box-warning" id='panel-add'>
	...
	<hr>
	<form method='post' action="{{ route('UpdateMenu') }}" enctype='multipart/form-data'>
		{{ csrf_field() }}
		<div class="box-body">
			<div class='col-lg-6 col-md-6 col-sm-12 col-xs-12 col-lg-offset-2 col-md-offset-2' style='padding:0px'>
				<div class="form-group">
					...
					<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
						<input type="hidden" name="id_menu" value='{{ $menu->id_menu }}' required="required" class="form-control input-sm customInput col-md-7 col-xs-12">
						<input type="text" name="nama_menu" value='{{ $menu->nama_menu }}' required="required" class="form-control input-sm customInput col-md-7 col-xs-12">
					</div>
				</div>
				...
				<div class="form-group" id="parent" style="display: none">
					...
					<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
						...
						<?php
						$menus_parent = DB::table('menu')->where('id_menu','!=',$menu->id_menu)->where('parent_id','0')->get();
						foreach ($menus_parent as $mp) {
							if($menu->parent_id==$mp->id_menu){
								?>
									<option value="{{$mp->id_menu}}" selected>{{$mp->nama_menu}}</option>
								<?php
							}else{
								?>
				...
				<div class="form-group">
					...
					<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
						<input type="radio" name="aktif" value='1' <?php if($menu->aktif == '1'){echo "checked";} ?> required="required" id='ya'><label for='ya' style='margin-right:10px;font-weight:normal'>Aktif</label>
						<input type="radio" name="aktif" value='0' <?php if($menu->aktif == '0'){echo "checked";} ?> required="required" id='tidak'><label for='tidak' style='font-weight:normal'>Tidak Aktif</label>
					</div>
				</div>
